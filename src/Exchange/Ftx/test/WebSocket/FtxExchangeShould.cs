...
namespace MarketIntelligency.Exchange.Ftx.Test
{
    public class FtxExchangeShould
    {
        ...
        [Fact]
        public async Task ReceiveOrderBookUpdate_update01()
        {
            //Arrange
            var cancellationToken = new CancellationToken();
            var logger = new Mock<ILogger<FtxExchange>>();
            ...
            Assert.Equal(orderbookResponse.Data.Asks.Count(), teste.Asks.ToList().Count);
            Assert.Equal(orderbookResponse.Data.Bids.Count(), teste.Bids.ToList().Count);
            Assert.True(!teste.Asks.Any(one => one.Price == (decimal)13615.0));
            Assert.Equal((decimal)15256.0, teste.Asks.ToList()[99].Price);
            Assert.Equal((decimal)0.001, teste.Asks.ToList()[99].Amount);
        }

        [Fact]
        public async Task ReceiveOrderBookUpdate_update02()
        {
            //Arrange
            var cancellationToken = new CancellationToken();
            var logger = new Mock<ILogger<FtxExchange>>();
            ...
            //Update 2
            payload = "{\"channel\": \"orderbook\", \"market\": \"ETH/BRZ\", \"type\": \"update\", \"data\": {\"time\": 1646003954.892236, \"checksum\": 2171692427, \"bids\": [], \"asks\": [[13628.0, 0.0], [15276.0, 0.001]], \"action\": \"update\"}}";
            lenght = 200;
            payloadAsByteArray = Encoding.ASCII.GetBytes(payload);
            webSocketClient.Setup(o => o.ReceiveAsync(cancellationToken))
                           .Returns(Task.FromResult(new WebSocketClientResponse()
            ...
            Assert.Equal(orderbookResponse.Data.Asks.Count(), teste.Asks.ToList().Count);
            Assert.Equal(orderbookResponse.Data.Bids.Count(), teste.Bids.ToList().Count);
            Assert.True(!teste.Asks.Any(one => one.Price == (decimal)13628.0));
            Assert.Equal((decimal)15276.0, teste.Asks.ToList()[99].Price);
            Assert.Equal((decimal)0.001, teste.Asks.ToList()[99].Amount);
        }

        [Fact]
        public async Task ReceiveOrderBookUpdate_update03()
        {
            //Arrange
            var cancellationToken = new CancellationToken();
            var logger = new Mock<ILogger<FtxExchange>>();
            ...
            //Update 3
            payload = "{\"channel\": \"orderbook\", \"market\": \"ETH/BRZ\", \"type\": \"update\", \"data\": {\"time\": 1646003955.0430365, \"checksum\": 837613493, \"bids\": [[13358.0, 13.6234]], \"asks\": [], \"action\": \"update\"}}";
            lenght = 186;
            payloadAsByteArray = Encoding.ASCII.GetBytes(payload);
            webSocketClient.Setup(o => o.ReceiveAsync(cancellationToken))
                           .Returns(Task.FromResult(new WebSocketClientResponse()
            ...
            OrderBook teste = result as OrderBook;
            Assert.Equal(orderbookResponse.Data.Asks.Count(), teste.Asks.ToList().Count);
            Assert.Equal(orderbookResponse.Data.Bids.Count() + 1, teste.Bids.ToList().Count);
            Assert.Equal((decimal)13358.0, teste.Bids.ToList()[17].Price);
            Assert.Equal((decimal)13.6234, teste.Bids.ToList()[17].Amount);
        }

        [Fact]
        public async Task ReceiveOrderBookUpdate_update04()
        {
            //Arrange
            var cancellationToken = new CancellationToken();
            var logger = new Mock<ILogger<FtxExchange>>();
            ...
            //Update 4
            payload = "{\"channel\": \"orderbook\", \"market\": \"ETH/BRZ\", \"type\": \"update\", \"data\": {\"time\": 1646003955.0953176, \"checksum\": 2092451951, \"bids\": [[13520.0, 0.0]], \"asks\": [], \"action\": \"update\"}}";
            lenght = 183;
            payloadAsByteArray = Encoding.ASCII.GetBytes(payload);
            webSocketClient.Setup(o => o.ReceiveAsync(cancellationToken))
                           .Returns(Task.FromResult(new WebSocketClientResponse()
            ...
            Assert.Equal(orderbookResponse.Data.Asks.Count(), teste.Asks.ToList().Count);
            Assert.Equal(orderbookResponse.Data.Bids.Count(), teste.Bids.ToList().Count);
            Assert.True(!teste.Bids.Any(one => one.Price == (decimal)13520.0));
        }
    }
}
